...
# Multi-panel timeseries of antecedent N deposition by plot and ecoregion

dat_plot <- df %>%
  select(plot_ID, date_m1, Dep_Nhistoric, ecoregion_ortho) %>%
  distinct(.) 
  
  fig2_supp <- ggplot(dat_plot, aes(x = date_m1, y = Dep_Nhistoric, group = plot_ID)) + 
...
ggsave(plot = fig2_supp, filename = "./visualizations/final_figures/FigureS6.tif",
