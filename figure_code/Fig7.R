...
letters_df <- final_pred_df %>%
  ...
  mutate(letters = paste0(letters[seq_len(nrow(fig7_eco))],"."))
...
spp_col <- data.frame(panel_fill_cols = colorblind_pal()(8),
                      species = unique(plot_data$species))
show_col(spp_col$panel_fill_cols)

strip_cols <- right_join(facet_label_df, spp_col, by = "species") %>%
  arrange(species, ecoregion) %>%
...
ggsave(p,filename = "./visualizations/final_figures/Figure7.png",
       device = "png", height = 15, width = 13, units = "in")
