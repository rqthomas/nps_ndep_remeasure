...
source("./figure_code/Fig6_supp_fig_function.R")
...
for(i in 1:length(spp_in_c1)){
  ...
  first_Ndep <- df %>%
    select(plot_ID, lat, lon, Dep_Nhistoric_SO, Dep_Ndiff_SO, date_m1, species) %>%
    filter(species == spp_in_c1[i]) %>%
    distinct(.) %>%
    group_by(plot_ID) %>%
    filter(year(date_m1) <= 2000) %>%
    ungroup()
  
  date_range_first_Ndep <- range(first_Ndep$date_m1, na.rm = TRUE)
  
  first_Ndep_sf <- st_as_sf(
    x = first_Ndep,
    coords = c("lon", "lat"), # Specify the longitude column first
    crs = crs_c1
  )
  
  first_fia_in_focal_c1 <- st_filter(first_Ndep_sf, focal_c1_poly)
  ...
  p1 <- ggplot() +
    ...
    geom_sf(data = first_fia_in_focal_c1, aes(color = Dep_Nhistoric_SO))+
    ylim(c(bbox_c1[2],bbox_c1[4]))+
    xlim(c(bbox_c1[1],bbox_c1[3]))+
    ggtitle(paste0("c. Early measurements: ",year(date_range_first_Ndep[1]),"-",year(date_range_first_Ndep[2])))+
    ylab("")+
    xlab("")+
    ...
    labs(x = bquote(atop(bold("observed res. ante. N dep. range:"),~~~~~~~~~~~~~~~~~~~~~~~~.(round(min(first_fia_in_focal_c1$Dep_Nhistoric_SO),2)) ~ "-" ~ .(round(max(first_fia_in_focal_c1$Dep_Nhistoric_SO),2)) ~ "kg N"~ha^-1~yr^1)))
  p1
  ...
  p2 <- ggplot() +
    ...
    ylim(c(bbox_c1[2],bbox_c1[4]))+
    xlim(c(bbox_c1[1],bbox_c1[3]))+
    ggtitle(paste0("d. Early measurements: ",year(date_range_first_Ndep[1]),"-",year(date_range_first_Ndep[2])))+
    ylab("")+
    xlab("")+
    ...
    theme(panel.border = element_rect(fill = NA, color = "darkgreen", linewidth = 2, linetype = "solid"),
          ...
          panel.grid.minor = element_blank(),
          panel.background = element_rect(fill = "lightcyan"))+
    labs(x = bquote(atop(bold("obs. res. ST change in N dep. range:"),~~~~~~~~~~~~~~~~~~~~~~~~~~.(round(min(first_fia_in_focal_c1$Dep_Ndiff_SO),2)) ~ "-" ~ .(round(max(first_fia_in_focal_c1$Dep_Ndiff_SO),2)) ~ "kg N"~ha^-1~yr^1)))
  p2
  ...
  p00 <- fig6_panels_to_use[[i]]+
    annotate("rect", 
             xmin = round(min(first_fia_in_focal_c1$Dep_Nhistoric_SO),2), 
             xmax = round(max(first_fia_in_focal_c1$Dep_Nhistoric_SO),2), 
             ymin = round(min(first_fia_in_focal_c1$Dep_Ndiff_SO),2), 
             ymax = round(max(first_fia_in_focal_c1$Dep_Ndiff_SO),2),
  ...
  ggsave(p, filename = paste0("./visualizations/final_figures/classI_all_3yr_window/",spp_in_c1[i],".png"),
