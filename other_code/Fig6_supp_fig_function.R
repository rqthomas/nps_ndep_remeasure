...
Fig6_supp_fig_function <- function(){
...
facet_label_df <- final_pred_df %>%
  ...
  mutate(er1 = str_replace_all(er1, " ", "~"),
         ...
         er1 = str_remove(er1, "~$"),
         er2 = str_remove(er2, "^~"),
         facet_labels = paste0("atop(bold(",letters,"~",genus,"~",spp,"), atop(atop(",er1, ",",er2,"),expected~growth:~",pred_baseline,"~kg~C~y^-1~ind^-1))")) %>%
...
for(j in 1:length(species_list)){
  ...
  plot_dat <- plot_data %>%
    ...
    plots[[l]] <- ggplot(data = plot_dat)+
      ...
      ylab(expression(paste("residual short-term change in N deposition (kg N ", ha^-1," ",yr^-1,")")))+
